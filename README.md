...
- `denormalize(t: torch.Tensor) -> torch.Tensor` — reverses the mean/std
  ...
  batched `(N, C, H, W)` tensor.
- `to_tensor_array(t: torch.Tensor, keep_normalized: bool = True) -> torch.Tensor` —
  returns a CNN-ready, **channel-first** tensor (`(C, H, W)` or batched
  `(N, C, H, W)`) — no permute, no numpy conversion, shape stays exactly
  what a CNN expects. `keep_normalized=True` (default) keeps the mean/std
  normalized values as-is (the correct input for training/inference);
  `keep_normalized=False` denormalizes to `[0, 1]` first but still returns
  channel-first (use this only if you need de-normalized values while
  staying in tensor form — for actual visualization, denormalize and
  permute manually, since this method intentionally never reorders axes).
...
- Every class above is importable straight from `neural_ed` (see the
  ...
  names (`fit_transform`, `process_path`, `to_tensor_array`, `summary`, etc.)
