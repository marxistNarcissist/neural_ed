...
class OrdinalEncoder(BaseEncoder):
    ...
    def fit(self, series: pd.Series) -> "OrdinalEncoder":
        ...
        if isinstance(self.categories, str) and self.categories == "auto":
            # dropna() so NaN never steals a code; sorted for determinism
            cats = pd.Series(series.dropna().unique())
            try:
                cats = cats.sort_values()
            except TypeError:                     # mixed, non-comparable types
                cats = cats.astype(str).sort_values()
    ...
    def transform(self, series: pd.Series) -> pd.Series:
        ...
        if unknown_mask.any():
            if self.handle_unknown == "error":
                ...
                raise ValueError(f"Found unknown categories during transform: {bad}")
